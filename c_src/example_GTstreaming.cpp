#include <GT_NN_SUM_STREAMING.hpp>
#include <bits/stdc++.h>
#include <cstdlib>
#include <unistd.h>
using namespace GT;
...
int main(int argc, char* argv[])
{
    ...
    int opt;
    while ((opt = getopt(argc, argv, "n:q:j:d:r:R:N:Q:D:")) != -1) {
        switch (opt) {
            case 'n':
                datasetName = optarg;
                break;
    ...
    std::cout << "Streaming runbook path: " << runbook_json << std::endl;
    ...
    search_engine.simulate_streaming(runbook_json);
    search_engine.simulate_streaming_exh(runbook_json);   
